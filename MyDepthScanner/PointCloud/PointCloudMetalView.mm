...
@implementation PointCloudMetalView {
    ...
    // Support multi-frame scanning
    simd::float4x4 _cameraToWorldMatrix;        // Store camera transformation
    NSMutableArray<NSValue *> *_cameraMatrices; // Store list of camera transformation each frame

    id<MTLCommandQueue> _commandQueue;
    id<MTLRenderPipelineState> _renderPipelineState;
    id<MTLRenderPipelineState> _unprojectPipelineState;
...
- (void)internalInit {
    ...
    // Initialize camera transformation's array
    _cameraMatrices = [NSMutableArray array];
...
- (void)configureMetal {
    ...
    // Load the vertex function from the library for point cloud
    id <MTLFunction> unprojectFunction = [defaultLibrary newFunctionWithName:@"unproject"];
    
    // Set up a descriptor for creating a pipeline state object
    MTLRenderPipelineDescriptor *unprojectPipelineStateDescriptor = [[MTLRenderPipelineDescriptor alloc] init];
    unprojectPipelineStateDescriptor.label = @"PointCloud Pipeline";
    unprojectPipelineStateDescriptor.vertexFunction = unprojectFunction;
    unprojectPipelineStateDescriptor.colorAttachments[0].pixelFormat = self.colorPixelFormat;
    unprojectPipelineStateDescriptor.depthAttachmentPixelFormat = MTLPixelFormatDepth32Float;
    unprojectPipelineStateDescriptor.rasterizationEnabled = false;
    
    NSError *error1 = NULL;
    _unprojectPipelineState = [self.device newRenderPipelineStateWithDescriptor:unprojectPipelineStateDescriptor
                                                                          error:&error1];

    if (!_unprojectPipelineState)
    {
        NSLog(@"Failed to created pipeline state for point cloud, error %@", error1);
    }
    
    // Set up buffer
//    NSInteger resultData[1] = {0};
//    _outBuffer = [self.device newBufferWithBytes:resultData
//                                          length:sizeof(resultData)
//                                         options:MTLResourceCPUCacheModeDefaultCache];
//    _particlesBuffer = [self.device newBufferWithLength:sizeof(TestParticle) options:MTLResourceStorageModeShared];
    
    
    // Initiate buffer for pointcloud
    //_pointCloudBuffer = [self.device newBufferWithLength:_pointCount * sizeof(PointData) options:MTLResourceStorageModeShared];
...
// Add new frame
- (void)addFrame:(AVDepthData* _Nonnull)depth
     withTecture:(_Nonnull CVPixelBufferRef)unormTexture
   cameraToWorld:(simd::float4x4)cameraToWorldMatrix {
    
    [self setDepthFrame:depth withTexture:unormTexture];
    
    // Save 'cameraToWorldMatrix' to the list
    @synchronized (self) {
        [_cameraMatrices addObject:[NSValue valueWithBytes:&cameraToWorldMatrix objCType:@encode(simd::float4x4)]];
    }
}
...
- (void)drawRect:(CGRect)rect {    
    ...
    _pointNumber = width*height;
    
    // Create buffer for storing point cloud data
    _pointCloudBuffer = [self.device newBufferWithLength:_pointNumber * sizeof(PointData) options:MTLResourceStorageModeShared];
    ...
    // Obtain a new cameraToWorldMatrix
    ...
    if(renderPassDescriptor != nil) {
        ...
        [renderEncoder setDepthStencilState:_depthStencilState];
        [renderEncoder setRenderPipelineState:_unprojectPipelineState];
        [renderEncoder setVertexTexture:depthTexture atIndex:0];

        [renderEncoder setVertexBytes:&finalViewMatrix length:sizeof(finalViewMatrix) atIndex:0];
        [renderEncoder setVertexBytes:&intrinsics length:sizeof(intrinsics) atIndex:1];
        [renderEncoder setVertexBuffer:_pointCloudBuffer offset:0 atIndex:2];
        ...
        /*[commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> commandBuffer) {
            void *contents = [_particlesBuffer contents];
            TestParticle *tp = (TestParticle *) contents;
            NSLog(@"data: %d %d", tp[0].number, tp[1].number);
        }];
         */
        
//        [commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> commandBuffer) {
//            /*PointData *pointCloudData = (PointData *)_pointCloudBuffer.contents;
//            for (NSUInteger i = 0; i < _pointNumber; i++) {
//                PointData point = pointCloudData[i];
//                // Now you can use `point.position` and `point.depth` as needed
//                NSLog(@"Point %lu - Position: (%f, %f, %f), Depth: %f", (unsigned long)i,
//                      point.position.x, point.position.y, point.position.z, point.depth);
//            }*/
//            
//        }];
        for (NSValue *value in _cameraMatrices) {
            simd::float4x4 cameraToWorldMatrix;
            [value getValue:&cameraToWorldMatrix];
            
            // Lakukan transformasi `cameraToWorldMatrix` ke titik cloud saat encoding
            [renderEncoder setVertexBytes:&cameraToWorldMatrix length:sizeof(cameraToWorldMatrix) atIndex:3];
            
            [renderEncoder drawPrimitives:MTLPrimitiveTypePoint
                              vertexStart:0
                              vertexCount:_pointNumber];
        }
    ...
    [_cameraMatrices removeAllObjects]; // Bersihkan setelah render
